...
findMin heap@(BinomialHeap size trees) = case trees of
    [] -> Nothing
    ts -> Just $ minimumBy (compare `on` fst) [(p, k) | t@(Node p k c) <- ts]
...
mergeTrees trees1 trees2 = foldr f [] $ zipExtend EmptyTree EmptyTree trees1 trees2
                      where f (EmptyTree, tree) acc = tree : acc
                            f (tree, EmptyTree) acc = tree : acc
                            f (tree1, tree2) acc = EmptyTree : insertTree (attach tree1 tree2) acc
{-
    *** TODO ***
...
isolate :: a -> [a] -> [(a, [a])]
isolate placeHolder [] = []
isolate placeHolder list = reverse $ ((fst (head (reverse result))), (reverse (tail (reverse (snd (head (reverse result))))))) : (tail (reverse result))
                          where 
                            result = snd $ foldr (\el (pos, final) -> (pos + 1, final ++ [(list !! pos, replace pos placeHolder list)])) (0, []) list
                              where
                                replace posish holder xs = a ++ [holder] ++ (tail b)
                                  where
                                    (a, b) = splitAt posish xs
-- [(,[]), (,[])]
...
removeMin heap@(BinomialHeap size trees) = BinomialHeap (size - 1) $ mergeTrees ts (reverse c)
            where
              Just prikey = findMin heap
              minTree@(Node pmin kmin cmin) = head $ filter (\aoleu@(Node pr ke ch) -> fst prikey == pr) trees
              (nod@(Node p k c), ts) = foldr doamne (EmptyTree, []) (isolate EmptyTree trees)
              doamne (tree, list) (accT, accL) = case accT of
                                                    EmptyTree -> (tree, list)
                                                    Node pAcc keyAcc childrenAcc -> case tree of
                                                              EmptyTree -> (accT, accL)
                                                              Node pTree keyTree childrenTree -> if pTree > pAcc
                                                                        then (accT, accL)
                                                                        else (tree, list) 
...
instance (Show p, Show k) => Show (BinomialTree p k) where
    show tree = case tree of
      EmptyTree -> "*"
      Node prio key children -> (show prio) ++ " (" ++ (show key) ++ ")" ++ varog
        where 
          varog = case children of
            [] -> ""
            _ -> "\n " ++ (intercalate " " (map (\child -> " " ++ (show child)) children))
...
instance (Show p, Show k) => Show (BinomialHeap p k) where
    show heap@(BinomialHeap size trees) = intercalate "\n" (map (\tree -> show tree) trees)
